icjm_nocorv <- function(df = 3, data, summary = T, n.adapt = 3000,
  ...
  for (i in 1:N_pt) {
    ...
    ### Treatment part
    ## Survival part
    log.s.trt.gk[i,1:qp] <- exp(1) ^ (T.trt.s[i,1:qp,] %*% gambh[,2] +
                                alpha[1,2] * (
    ...
    ## Hazard part
    log.h.trt[i] <- inprod(T.trt.h[i, ], gambh[,2]) + 
                  alpha[1,2] * (
    ...
    ## Survival part
    log.s.prg.gk[i,1:qp] <- exp(1) ^ (T.prg.s[i,1:qp,] %*% gambh[,1] + 
                                alpha[1,1] * (
    ...
    ## Survival part
    for (m in 1:qp) {
      log.s.prginc.int.gk.gk[i,m,1:qp] <- exp(1) ^ (t(T.prginc.s[i,m,,1:qp]) %*% gambh[,1] +
    ...
    h.prginc.int.gk[i,1:qp] <- exp(1) ^ (T.prginc.h[i,1:qp,] %*% gambh[,1] + 
                                 alpha[1,1] * (
  ...
  data <- list(N_pt = N_pt, qp = qp, num = num, Y = Y, XL = XL, ZL = ZL,
               T.trt.s = T.trt.s, XL.trt.s = XL.trt.s, ZL.trt.s = ZL.trt.s,
               evt.trt = evt.trt, w = w, 
