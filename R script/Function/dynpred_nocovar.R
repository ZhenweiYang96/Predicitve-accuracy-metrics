...
csdypred <- function(model = NULL,
  ...
  jags_res <- extract_pd(object$mcmc, covar = F)
  ...
  for (j in 1:iter) {
    hazard.survival[j,1:qp,1:2] <- exp(T.hs[1:qp,] %*% gam_bh[j,,1:2] +
  ...
  for (i in 1:length(step)) {
    ...
    for (j in 1:iter) {
      for (m in 1:qp) {
        survival[j,m] <- exp(
          - t.step[m]/2 * sum(w %*% exp(t(T.s[m,,1:qp]) %*% gam_bh[j,,1:2]  + 
      ...
      hazard.overall.survival[j,1:qp,1:2] <- exp(T.os[1:qp,] %*% gam_bh[j,,1:2] + 
      ...
      hazard[j,1:qp] <- exp(T.h[1:qp,] %*% gam_bh[j,,1] +  
  ...
  if (trt_part) {
    ...
    for (j in 1:iter) {
      ...
      hazard.overall.survival[j,1:qp,1:2] <- exp(T.os[1:qp,] %*% gam_bh[j,,1:2] + 
                                                   ...
                                                   (XL.os.now[1:qp,] %*% beta[j,] + ZL.os.now[1:qp,] %*% b[j,] -
                                                      XL.os.back[1:qp,] %*% beta[j,] - ZL.os.back[1:qp,] %*% b[j,]) %*% alpha[j,2,1:2])
      hazard[j,1:qp] <- exp(T.h[1:qp,] %*% gam_bh[j,,2] +  
