...
csdypred <- function(model = NULL,
  ...
  ZL.b.now <- matrix(1, qp, sqrt(ncol(pool.D)))
  XL.b.now[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
  XL.b.now[,2:(sqrt(ncol(pool.D)))] <- ZL.b.now[,2:(sqrt(ncol(pool.D)))] <- f.org * t/2 + t/2
  
  XL.b.back <- matrix(1, qp, ncol(pool.beta))
  ZL.b.back <- matrix(1, qp, sqrt(ncol(pool.D)))
  XL.b.back[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
  XL.b.back[,2:(sqrt(ncol(pool.D)))] <- ZL.b.back[,2:(sqrt(ncol(pool.D)))] <- f.org * t/2 + t/2 - 1
  ...
  loglike <- function(b) {
    n_test/2 * log(tau[i,] / (3*pi))  - 2 * sum(sapply(1:n_test, function(n) {
      ...
      0.5 * log(det(Sigma[i,,])) - 0.5 * matrix(b,1, sqrt(ncol(pool.D))) %*% solve(Sigma[i,,]) %*% matrix(b,sqrt(ncol(pool.D)),1)
  }
  ...
  d=sqrt(ncol(pool.D)) #dimension of parameters to be updated
  ...
  ZL.hs.now <- matrix(1,qp, sqrt(ncol(pool.D)))
  XL.hs.now[,2:(sqrt(ncol(pool.D)))] <- ZL.hs.now[,2:(sqrt(ncol(pool.D)))] <- t(sapply(1:qp, function(n) {
    f.org[n] * t.start/2 + t.start/2
  }))
  XL.hs.now[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
  
  XL.hs.back <- matrix(1, qp, ncol(pool.beta))
  ZL.hs.back <- matrix(1, qp, sqrt(ncol(pool.D)))
  XL.hs.back[,2:(sqrt(ncol(pool.D)))] <- ZL.hs.back[,2:(sqrt(ncol(pool.D)))] <- t(sapply(1:qp, function(n) {
    f.org[n] * t.start/2 + t.start/2 - 1
  }))
  XL.hs.back[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
  ...
  for (i in 1:length(step)) {
    ...
    ZL.h.now <- matrix(1, qp, sqrt(ncol(pool.D)))
    XL.h.now[,2:(sqrt(ncol(pool.D)))] <- ZL.h.now[,2:(sqrt(ncol(pool.D)))] <- f.org * (u-t)/2 + (u+t)/2
    XL.h.now[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
    
    XL.h.back <- matrix(1, qp, ncol(pool.beta))
    ZL.h.back <- matrix(1, qp, sqrt(ncol(pool.D)))
    XL.h.back[,2:(sqrt(ncol(pool.D)))] <- ZL.h.back[,2:(sqrt(ncol(pool.D)))] <- f.org * (u-t)/2 + (u+t)/2 - 1
    XL.h.back[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
    ...
    ZL.s.now <- array(1, dim = c(qp, sqrt(ncol(pool.D)), qp))
    XL.s.now[,(sqrt(ncol(pool.D))+1):ncol(pool.beta),] <- test.id[[longi.bsVar]]
    
    for (j in 1:qp) {
      XL.s.now[j,2:(sqrt(ncol(pool.D))),] <- ZL.s.now[j,2:(sqrt(ncol(pool.D))),] <- t(f.org * t.step[j]/2 + t.step[j]/2)
    }
    
    XL.s.back <- array(1, dim = c(qp, ncol(pool.beta), qp))
    ZL.s.back <- array(1, dim = c(qp, sqrt(ncol(pool.D)), qp))
    XL.s.back[,(sqrt(ncol(pool.D))+1):ncol(pool.beta),] <- test.id[[longi.bsVar]]
    
    for (j in 1:qp) {
      XL.s.back[j,2:(sqrt(ncol(pool.D))),] <- ZL.s.back[j,2:(sqrt(ncol(pool.D))),] <- t(f.org * t.step[j]/2 + t.step[j]/2 - 1)
    }
    ...
    ZL.os.now <- matrix(1,qp, sqrt(ncol(pool.D)))
    XL.os.now[,2:(sqrt(ncol(pool.D)))] <- ZL.os.now[,2:(sqrt(ncol(pool.D)))] <- t(sapply(1:qp, function(n) {
      f.org[n] * u/2 + u/2
    }))
    XL.os.now[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
    
    XL.os.back <- matrix(1,qp, ncol(pool.beta))
    ZL.os.back <- matrix(1,qp, sqrt(ncol(pool.D)))
    XL.os.back[,2:(sqrt(ncol(pool.D)))] <- ZL.os.back[,2:(sqrt(ncol(pool.D)))] <- t(sapply(1:qp, function(n) {
      f.org[n] * u/2 + u/2 - 1
    }))
    XL.os.back[,(sqrt(ncol(pool.D))+1):ncol(pool.beta)] <- test.id[[longi.bsVar]]
  ...
  longi[,2] <- sapply(seq(0, max(test$TimeSince_Dx), length.out = 50), function(n) {
    mean(sapply(1:iter, function(l) {
      c(1, n, test.id$DxAge) %*% beta[l,] +
        c(1, n) %*% b[l,]
    }))
  })
  
  if (trt_part) {
    inf_mean <- mean(rowSums(risk.store)/(rowSums(risk.store) + risk.store_trt), na.rm = T)
    inf_ci <- quantile(rowSums(risk.store)/(rowSums(risk.store) + risk.store_trt), 
                       probs= c(0.975, 0.025), na.rm = T)
